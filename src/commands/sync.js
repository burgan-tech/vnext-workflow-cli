...
const { getJsonMetadata, detectComponentType, checkComponentDomain } = require('../lib/workflow');
...
async function syncCommand(options) {
  ...
  await runForEachSolution(options, {}, syncSolution);
}

async function syncSolution(solution) {
  const profile = solution.profile;

  if (!profile.AUTO_DISCOVER) {
  ...
  if (totalSuccess > 0) {
    ...
    if (hookResult.success) {
      ...
      hookSpinner.warn(chalk.yellow(`Publish-completed reported a problem: ${hookResult.error}`));
    }
  ...
  for (const [type, folderName] of Object.entries(solution.componentTypes)) {
